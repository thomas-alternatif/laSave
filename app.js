  ...
  function openOrgaModal(orga){
    ...
    if(orga.contact){
      ...
      const icoIG=`<img src="${$('link[rel=icon]')?.href||''}" width="14" height="14" style="display:none"/>`;
      html+=`<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:1.5rem;padding-bottom:1.5rem;border-bottom:1px solid var(--border);">`;
      parts.forEach(p=>{
        let href=p.startsWith('http')?p:'https://'+p;
        let label=href.replace(/^https?:\/\/(www\.)?/,'').replace(/\/$/,'');
        html+=`<a href="${esc(href)}" target="_blank" rel="noopener" style="font-size:12px;color:var(--gold);text-decoration:none;">${esc(label)}</a>`;
      });
      html+=`</div>`;
    }
