/* laSave — test10 : à la une, rendez-vous, rubriques, organisateurs */
(() => {
  'use strict';
  ...
  function buildFlow(events) {
    ...
    const list = events.filter(e => e.Date && !['Conférence / Atelier', 'Sport / Loisir'].includes(e['Catégorie'])).slice(0, 12);
  ...
  function buildRows(events) {
    ...
    const secs = RUBRIQUES.map(r => ({ ...r, evs: events.filter(e => r.cats.includes(e['Catégorie']) || (r.cats.includes('Autre') && !known.includes(e['Catégorie']))) })).filter(r => r.evs.length);
    if (!secs.length) { $('#envies').hidden = true; return; }
    let cur = 0, commune = null, day = null, open = false;
    const rubEl = $('#envies'), tabs = $('#rub-tabs'), grid = $('#rub-grid'), all = $('#rub-all');
    // Onglets
    const tabBtns = secs.map((r, i) => {
      const b = el('button', 'rub-tab'); b.type = 'button'; b.setAttribute('role', 'tab'); b.id = 'tab-' + i;
      b.appendChild(document.createTextNode(r.tab)); b.appendChild(el('small', null, `${r.evs.length} à venir`));
      b.addEventListener('click', () => { cur = i; render(); });
      b.addEventListener('keydown', ev => { if (ev.key === 'ArrowRight' || ev.key === 'ArrowLeft') { ev.preventDefault(); cur = (cur + (ev.key === 'ArrowRight' ? 1 : -1) + secs.length) % secs.length; render(); tabBtns[cur].focus(); } });
      tabs.appendChild(b); return b;
    });
    // Communes
    const communes = [...new Set(events.map(e => e.Commune).filter(Boolean))].sort((x, y) => x.localeCompare(y, 'fr'));
    const abbr = c => c.split(/[\s-]+/).filter(w => !/^(sur|de|la|le|les|du|en)$/i.test(w)).map(w => w[0]).join('').slice(0, 3).toUpperCase();
    const cBox = $('#side-communes');
    const cBtns = communes.map(c => {
      const b = el('button', 'side-commune'); b.type = 'button';
      b.appendChild(el('b', null, abbr(c))); b.appendChild(document.createTextNode(c.replace(/-sur-Save$/, '')));
      b.setAttribute('aria-label', c);
      b.addEventListener('click', () => { commune = commune === c ? null : c; render(); });
      cBox.appendChild(b); return [c, b];
    });
    // Jours
    const dBox = $('#side-days');
    const dBtns = [];
    for (let k = -1; k < 7; k++) {
      let d = null, lab, big;
      if (k < 0) { lab = 'Tous'; big = '∞'; }
      else { d = new Date(today()); d.setDate(d.getDate() + k); lab = k === 0 ? 'Auj.' : k === 1 ? 'Dem.' : d.toLocaleDateString('fr-FR', { weekday: 'short' }).replace('.', ''); big = String(d.getDate()); }
      const b = el('button', 'side-day'); b.type = 'button';
      b.appendChild(document.createTextNode(lab.charAt(0).toUpperCase() + lab.slice(1))); b.appendChild(el('b', null, big));
      if (d) b.setAttribute('aria-label', d.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' }));
      b.addEventListener('click', () => { day = d; render(); });
      dBox.appendChild(b); dBtns.push([d, b]);
    }
    $('#side-reset').addEventListener('click', () => { commune = null; day = null; render(); });
    all.addEventListener('click', () => { open = !open; render(); });
    function render() {
      const r = secs[cur];
      rubEl.style.setProperty('--c', r.color);
      $('#rub-kicker').textContent = r.kicker;
      $('#rub-title').textContent = r.title;
      const withPhoto = r.evs.find(e => e.Photo && e.Photo[0]) || r.evs[0];
      bg($('#rub-img'), photoOf(withPhoto));
      tabBtns.forEach((b, i) => { b.setAttribute('aria-selected', String(i === cur)); b.tabIndex = i === cur ? 0 : -1; });
      grid.setAttribute('aria-labelledby', 'tab-' + cur);
      const inRub = e => r.evs.includes(e);
      cBtns.forEach(([c, b]) => { b.setAttribute('aria-pressed', String(c === commune)); b.style.opacity = r.evs.some(e => e.Commune === c) ? '1' : '.45'; });
      dBtns.forEach(([d, b]) => {
        b.setAttribute('aria-pressed', String((d && day && +d === +day) || (!d && !day)));
        if (d) b.classList.toggle('empty', !r.evs.some(e => onDay(e, d) && (!commune || e.Commune === commune)));
      });
      const list = events.filter(e => inRub(e) && (!commune || e.Commune === commune) && (!day || onDay(e, day)));
      const bits = [];
      if (day) bits.push(day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric' }));
      if (commune) bits.push(commune);
      $('#rub-sub').textContent = bits.length ? `À l'affiche · ${bits.join(' · ')}` : "À l'affiche";
      grid.textContent = '';
      grid.classList.toggle('open', open);
      if (!list.length) grid.appendChild(el('p', 'rub-empty', 'Rien de prévu avec ces choix. Essayez un autre jour ou une autre commune.'));
      list.forEach((e, i) => { const p = poster(e); if (i >= 8) p.classList.add('extra'); grid.appendChild(p); });
      all.hidden = list.length <= 8;
      all.textContent = open ? 'Réduire' : `Voir tout (${list.length})`;
